...
app.post("/api/login", async (req, res) => {
  ...
  const token = jwt.sign(
    { id: user.id, email: user.email, isAdmin: user.is_admin, isSeller: user.is_seller },
    JWT_SECRET,
    { expiresIn: "1h" }
  );
  res.json({ token });
...
app.get("/api/profile", authMiddleware, async (req, res) => {
  const result = await pool.query("SELECT id, email, is_admin, is_seller FROM users WHERE id=$1", [req.user.id]);
  res.json(result.rows[0]);
});

// ================= ADMIN: CREAR VENDEDOR =================
app.post("/api/admin/create-seller", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      "INSERT INTO users (email, password_hash, is_seller) VALUES ($1, $2, true) RETURNING id",
      [email, passwordHash]
    );
    const sellerId = result.rows[0].id;

    await pool.query("INSERT INTO stores (name, seller_user_id) VALUES ($1, $2)", [storeName, sellerId]);

    res.json({ message: "Seller created", sellerId });
  } catch (err) {
...
// Ruta para que el vendedor añada productos a su tienda
app.post("/api/seller/products", authMiddleware, async (req, res) => {
  if (!req.user.isSeller) return res.status(403).json({ error: "Not authorized" });

  const { name, price, price_xaf } = req.body;
  const storeRes = await pool.query("SELECT id FROM stores WHERE seller_user_id=$1", [req.user.id]);
  if (storeRes.rows.length === 0) return res.status(400).json({ error: "Store not found for this seller" });

  const storeId = storeRes.rows[0].id;
  const result = await pool.query(
    "INSERT INTO products (name, price, price_xaf, store_id) VALUES ($1, $2, $3, $4) RETURNING *",
    [name, price, price_xaf, storeId]
  );
...
// ================= HEALTH CHECK =================
app.get("/", (req, res) => {
  res.send("✅ API WapMarket funcionando");
});
